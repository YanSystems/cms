...
import (
	...
	"github.com/YanSystems/cms/pkg/api"
...
var testsCollection = uuid.New().String()

func (chtc *ContentHandlerTestCase) RunCreateContentTest(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "/contents/"+chtc.CollectionName, bytes.NewBuffer(payload))
	assert.NoError(t, err)
